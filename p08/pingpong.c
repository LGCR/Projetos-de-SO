...
task_t *last_task, *current_task, *task_resume_join;
...
int task_isActive(task_t* task){
    return task->task_isActive;
}
...
void task_exit(int exitCode) {
    current_task->task_isActive = 0;
    current_task->task_exitCode = exitCode;
...
void task_suspend(task_t *task, task_t **queue) {
    if(task != NULL){
        queue_append((queue_t**) &suspend_queue, (queue_t*) current_task);
    }
}
...
void task_resume(task_t *task) {
    task_t *task_removed = NULL;
    if(task != NULL){
        if(queue_size((queue_t*) suspend_queue) > 0){
...
int task_join(task_t *task) {
    if(task != NULL){
        if(task_isActive(task)) {
            task_suspend(current_task, &ready_queue);
            task->task_join = 1;
            task_resume_join = current_task;
            task_switch(task);
            task_resume_join = 0;
            task_resume(task_resume_join);
            return task->task_exitCode;
        }
    }
    return -1;
}
